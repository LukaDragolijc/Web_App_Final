...
def index():
    user = {'username': 'Male Oscar Winners Web App Project'}
    cursor = mysql.connect().cursor()
    cursor.execute('SELECT * FROM tblMaleOscarWinners')
...
def record_view(actor_id):
    ...
    cursor.execute('SELECT * FROM tblMaleOscarWinners WHERE id=%s', actor_id)
    result = cursor.fetchall()
...
def form_update_post(actor_id):
    ...
    sql_update = """UPDATE tblMaleOscarWinners t SET t.Name = %s, t.Age = %s, t.Year = %s, t.Movie = %s WHERE t.id = %s """
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO tblMaleOscarWinners (Year, Age, Name, Movie) VALUES (%s, %s, %s, %s) """
...
def form_delete_post(actor_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM tblMaleOscarWinners WHERE id = %s """
...
def api_edit(actor_id) -> str:
    ...
    sql_update = """UPDATE tblMaleOscarWinners t SET t.Year = %s, t.Age = %s, t.Name = %s, t.Movie = %s WHERE t.id = %s"""
